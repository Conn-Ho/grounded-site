...
export default function Waitlist({
  ...
  const [btnWidth, setBtnWidth] = useState<number | undefined>();

  useEffect(() => {
    const measure = () => {
      if (spanRef.current) {
        setBtnWidth(spanRef.current.getBoundingClientRect().width);
      }
    };
    measure();
  ...
  return (
    <section id="waitlist" className="snap-section relative flex flex-col justify-center py-24 md:py-28">
      <div className="mx-auto w-full max-w-[900px] px-6 md:px-10 lg:px-16 2xl:max-w-[1000px] text-center">
        ...
        <h2 className="display-font text-[clamp(2rem,4.5vw,3.6rem)] leading-[1.02]">
          {dict.titleLine1}
          <br />
          <span ref={spanRef} className="text-[color:var(--accent)]">
            {dict.titleLine2}
          </span>
        </h2>
        ...
        <div className="mt-10 flex justify-center">
          <a
            ...
            className="group inline-flex h-14 items-center justify-center gap-3 rounded-xl bg-[color:var(--accent)] px-10 mono text-[13px] uppercase tracking-[0.15em] text-white font-semibold shadow-[0_0_32px_rgba(246,83,16,0.35)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_0_48px_rgba(246,83,16,0.5)] hover:opacity-95 whitespace-nowrap"
          >
            {dict.submit}
