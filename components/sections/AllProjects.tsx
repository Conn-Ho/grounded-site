...
export default function AllProjects({
  ...
  const projectItems = huntItems.filter((i) => i.type === "project");
      ...
      <div className="mx-auto w-full max-w-[1400px] px-6 md:px-10 lg:px-16">
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          ...
          {galleryDict.projects.map((p, idx) => (
            ...
            >
              <div className={`relative aspect-[4/3] overflow-hidden bg-gradient-to-br ${covers[idx % covers.length]}`}>
                <svg className="absolute inset-0 h-full w-full opacity-40" aria-hidden>
                  <defs>
                    <pattern id={`dots-all-${p.slug}`} width="12" height="12" patternUnits="userSpaceOnUse">
                      <circle cx="1.5" cy="1.5" r="1" fill="var(--accent)" />
                    </pattern>
                  </defs>
                  <rect width="100%" height="100%" fill={`url(#dots-all-${p.slug})`} />
                </svg>
              ...
              <div className="p-5">
                ...
                <p className="mb-4 text-[13px] leading-relaxed text-[color:var(--foreground-dim)]">{p.description}</p>
            ...
            return (
              ...
              >
                <div className={`relative aspect-[4/3] overflow-hidden bg-gradient-to-br ${cover}`}>
                  <svg className="absolute inset-0 h-full w-full opacity-40" aria-hidden>
                    <defs>
                      <pattern id={`dots-all-hunt-${item.id}`} width="12" height="12" patternUnits="userSpaceOnUse">
                        <circle cx="1.5" cy="1.5" r="1" fill="var(--accent)" />
                      </pattern>
                    </defs>
                    <rect width="100%" height="100%" fill={`url(#dots-all-hunt-${item.id})`} />
                  </svg>
