...
const FROM = process.env.EMAIL_FROM ?? "Grounded <hello@grounded.dev>";

const copy = {
  en: {
    subject: "You're on the Grounded waitlist",
    preview: "We'll reach out as soon as your slot opens.",
    greeting: "Welcome,",
    body: [
      "You just joined the Grounded waitlist — the missing tooling layer that lets AI agents design, source, build, and verify real hardware.",
    ...
    footer: "— The Grounded team",
  },
  zh: {
    subject: "你已加入 Grounded 内测名单",
    preview: "名额开放后我们会第一时间联系你。",
    greeting: "你好，",
    body: [
      "你刚刚加入了 Grounded 内测名单 —— 我们正在造一层让 AI Agent 能独立完成硬件设计、采购、制造、验证的工具层。",
...
function renderText(locale: Locale) {
  const c = copy[locale];
  return [c.greeting, "", ...c.body, "", c.footer, "", "grounded.dev"].join("\n");
}
...
export async function sendWelcomeEmail(
  ...
  try {
    const { error } = await r.emails.send({
      ...
      headers: { "X-Grounded-List": "waitlist" },
    });
